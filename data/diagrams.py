...
def _wrap_svg_lines(text: str, max_chars: int = 28, max_lines: int = 4) -> list[str]:
    words = text.split()
    lines: list[str] = []
    current = ""
    for word in words:
    ...
    if len(lines) > max_lines:
        ...
        trimmed.append((overflow[: max_chars - 3]).rstrip() + "...")
        return trimmed
    return lines
...
def _svg_text_block(
    ...
    return f'<text class="{css_class}" x="{x}" y="{y}" text-anchor="{anchor}">{"".join(tspans)}</text>'


def _mind_map_subpoints(text: str, limit: int = 3) -> list[str]:
    normalized = (
        text.replace(": ", ", ")
        .replace("; ", ", ")
        .replace(" and ", ", ")
    ...
    points: list[str] = []
    for part in parts:
        lines = _wrap_svg_lines(part, max_chars=20, max_lines=2)
        points.append(" ".join(lines))
        if len(points) >= limit:
            break
    if not points:
        points = ["Key detail"]
...
<defs><marker id="a" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto"><path d="M2 2L8 5L2 8" fill="none" stroke="var(--dm,#8892a4)" stroke-width="1.5"/></marker></defs>
<text class="h" x="340" y="22" text-anchor="middle" font-size="15">Five war stories â€” each answers a different question</text>
<rect x="40" y="42" width="280" height="42" rx="8" fill="#D85A3020" stroke="#D85A30" stroke-width="0.8"/>
<text class="h" x="180" y="58" text-anchor="middle" fill="#D85A30" font-size="12">#1 Set vs List bug</text>
<text class="s" x="180" y="74" text-anchor="middle">Code review + team education</text>
<line class="arr" x1="320" y1="63" x2="368" y2="63"/>
<rect x="370" y="42" width="270" height="42" rx="8" fill="#D85A3008" stroke="#D85A3060" stroke-width="0.5"/>
<text class="s" x="505" y="60" text-anchor="middle" fill="#D85A30">Dependency mgmt, mentorship</text>
<text class="s" x="505" y="75" text-anchor="middle">Use: "subtle bug you found"</text>

<rect x="40" y="96" width="280" height="42" rx="8" fill="#BA751720" stroke="#BA7517" stroke-width="0.8"/>
<text class="h" x="180" y="112" text-anchor="middle" fill="#BA7517" font-size="12">#2 Folium token spike</text>
<text class="s" x="180" y="128" text-anchor="middle">Hidden resource drain</text>
<line class="arr" x1="320" y1="117" x2="368" y2="117"/>
<rect x="370" y="96" width="270" height="42" rx="8" fill="#BA751708" stroke="#BA751760" stroke-width="0.5"/>
<text class="s" x="505" y="114" text-anchor="middle" fill="#BA7517">Cost debugging, root cause</text>
<text class="s" x="505" y="129" text-anchor="middle">Use: "cost/performance issue"</text>

<rect x="40" y="150" width="280" height="42" rx="8" fill="#7F77DD20" stroke="#7F77DD" stroke-width="0.8"/>
<text class="h" x="180" y="166" text-anchor="middle" fill="#7F77DD" font-size="12">#3 Data masking (Mostly.ai)</text>
<text class="s" x="180" y="182" text-anchor="middle">PII protection + deployment</text>
<line class="arr" x1="320" y1="171" x2="368" y2="171"/>
<rect x="370" y="150" width="270" height="42" rx="8" fill="#7F77DD08" stroke="#7F77DD60" stroke-width="0.5"/>
<text class="s" x="505" y="168" text-anchor="middle" fill="#7F77DD">Privacy, compliance, strategy</text>
<text class="s" x="505" y="183" text-anchor="middle">Use: "sensitive data handling"</text>

<rect x="40" y="204" width="280" height="42" rx="8" fill="#378ADD20" stroke="#378ADD" stroke-width="0.8"/>
<text class="h" x="180" y="220" text-anchor="middle" fill="#378ADD" font-size="12">#4 Schema drift (Evidently AI)</text>
<text class="s" x="180" y="236" text-anchor="middle">Feature drift + validation</text>
<line class="arr" x1="320" y1="225" x2="368" y2="225"/>
<rect x="370" y="204" width="270" height="42" rx="8" fill="#378ADD08" stroke="#378ADD60" stroke-width="0.5"/>
<text class="s" x="505" y="222" text-anchor="middle" fill="#378ADD">MLOps maturity, prevention</text>
<text class="s" x="505" y="237" text-anchor="middle">Use: "pipeline failure"</text>

<rect x="40" y="258" width="280" height="42" rx="8" fill="#1D9E7520" stroke="#1D9E75" stroke-width="0.8"/>
<text class="h" x="180" y="274" text-anchor="middle" fill="#1D9E75" font-size="12">#5 API timeout handling</text>
<text class="s" x="180" y="290" text-anchor="middle">UX resilience</text>
<line class="arr" x1="320" y1="279" x2="368" y2="279"/>
<rect x="370" y="258" width="270" height="42" rx="8" fill="#1D9E7508" stroke="#1D9E7560" stroke-width="0.5"/>
<text class="s" x="505" y="276" text-anchor="middle" fill="#1D9E75">System resilience, UX empathy</text>
<text class="s" x="505" y="291" text-anchor="middle">Use: "improve reliability"</text>

<rect x="100" y="314" width="480" height="22" rx="6" fill="none" stroke="var(--brd,#2a3040)" stroke-width="0.5" stroke-dasharray="4 3"/>
<text class="s" x="340" y="329" text-anchor="middle">Each story is 30-45 seconds â€” keep all five ready for behavioral rounds</text>
</svg>'''


def story_mind_map_diagram(story: dict) -> str:
    branches = story.get("mind_map", [])[:5]
    if not branches:
        return ""
    ...
    center_h = 142
    positions = [
        {"x": 530, "y": 160, "attach": "bottom", "layout": "top"},
        {"x": 780, "y": 248, "attach": "left", "layout": "right"},
        {"x": 720, "y": 438, "attach": "left", "layout": "lower-right"},
        {"x": 340, "y": 438, "attach": "right", "layout": "lower-left"},
        {"x": 280, "y": 248, "attach": "right", "layout": "left"},
    ]
    palette = [
        {"fill": "#f6b7ef", "stroke": "#d95dc8"},
        {"fill": "#ffc0b4", "stroke": "#f27f65"},
        {"fill": "#d9f38a", "stroke": "#92bb1f"},
        {"fill": "#ffe48f", "stroke": "#d4a100"},
        {"fill": "#d6bef8", "stroke": "#9f6df0"},
    ]

    center_lines = _wrap_svg_lines(story.get("title", "Mind Map"), max_chars=18, max_lines=3)
    center_svg = [
        f'<ellipse cx="{center_x}" cy="{center_y}" rx="{center_w // 2}" ry="{center_h // 2}" fill="#0b2341" stroke="#e86100" stroke-width="3"/>',
        _svg_text_block(center_lines, center_x, center_y - 18, "ct", 18),
        _svg_text_block(["Interview expansion map"], center_x, center_y + 42, "cs", 14),
    ]
    ...
    for index, branch in enumerate(branches):
        ...
        if pos["attach"] == "bottom":
            attach_x = px
            attach_y = py + half_h
        elif pos["attach"] == "left":
            attach_x = px - half_w
            attach_y = py
        else:
            attach_x = px + half_w
        ...
        mid_x = round(center_x + (attach_x - center_x) * 0.45)
        mid_y = round(center_y + (attach_y - center_y) * 0.45)

        branch_svg.append(
            f'<path d="M {center_x} {center_y} L {mid_x} {mid_y} L {attach_x} {attach_y}" '
            f'stroke="{color["stroke"]}" stroke-width="8" fill="none" stroke-linecap="round" stroke-linejoin="round"/>'
        )
        branch_svg.append(
            f'<circle cx="{mid_x}" cy="{mid_y}" r="9" fill="{color["stroke"]}" stroke="#ffffff" stroke-width="3"/>'
        )
        branch_svg.append(
            f'<rect x="{px - half_w}" y="{py - half_h}" width="{card_w}" height="{card_h}" rx="12" '
            f'fill="{color["fill"]}" stroke="{color["stroke"]}" stroke-width="2"/>'
        )
        branch_svg.append(
            _svg_text_block(
                _wrap_svg_lines(branch.get("label", ""), max_chars=18, max_lines=2),
                px,
                py - 6,
                "bh",
                15,
        ...
        subpoints = _mind_map_subpoints(branch.get("text", ""), limit=3)
        if pos["layout"] == "top":
            child_positions = [
                (px - 190, py - 108),
                (px, py - 128),
        ...
        elif pos["layout"] == "right":
            child_positions = [
                (px + 170, py - 82),
                (px + 182, py),
                (px + 170, py + 82),
            ]
        elif pos["layout"] == "lower-right":
            child_positions = [
                (px + 155, py - 32),
                (px + 150, py + 52),
                (px + 70, py + 108),
            ]
        elif pos["layout"] == "lower-left":
            child_positions = [
                (px - 155, py - 32),
                (px - 150, py + 52),
                (px - 70, py + 108),
        ...
        branch_attach = {
            "top": (px, py - half_h),
            "right": (px + half_w, py),
            "lower-right": (px + half_w, py),
            "lower-left": (px - half_w, py),
            "left": (px - half_w, py),
        }[pos["layout"]]

        for sub_index, subpoint in enumerate(subpoints):
            child_x, child_y = child_positions[sub_index]
            line_end_x = child_x - child_w // 2 if child_x > px else child_x + child_w // 2
            line_end_y = child_y
            if pos["layout"] == "top":
                line_end_x = child_x
                line_end_y = child_y + child_h // 2
            elif pos["layout"] in {"lower-right", "lower-left"}:
                line_end_y = child_y - child_h // 2

            branch_svg.append(
                f'<path d="M {branch_attach[0]} {branch_attach[1]} L {line_end_x} {line_end_y}" '
                f'stroke="{color["stroke"]}" stroke-width="5" fill="none" stroke-linecap="round"/>'
            )
            branch_svg.append(
                f'<rect x="{child_x - child_w // 2}" y="{child_y - child_h // 2}" width="{child_w}" height="{child_h}" rx="10" '
                f'fill="{color["fill"]}" stroke="{color["stroke"]}" stroke-width="1.8"/>'
            )
            branch_svg.append(
                _svg_text_block(
                    _wrap_svg_lines(subpoint, max_chars=20, max_lines=2),
                    child_x,
                    child_y - 4,
                    "st",
                    13,
...
<style>
.bg{{fill:#edf1f7;stroke:#d2dbe8;stroke-width:1.2}}
.ct{{font:700 28px 'Source Sans 3',sans-serif;fill:#ffffff}}
.cs{{font:500 13px 'IBM Plex Mono',monospace;fill:#d9e5f8;letter-spacing:0.5px;text-transform:uppercase}}
.bh{{font:700 16px 'Source Sans 3',sans-serif;fill:#17202d}}
.st{{font:500 12px 'Source Sans 3',sans-serif;fill:#293548}}
</style>
<rect class="bg" x="6" y="6" width="{width - 12}" height="{height - 12}" rx="20"/>
{''.join(branch_svg)}
{''.join(center_svg)}
</svg>'''
    )
